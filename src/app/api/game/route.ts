...
async function generateNarrativeWithLLM(openai: OpenAI, state: GameState, userInput: string): Promise<string> {
    ...
    const systemPrompt = `
        ...
        - **정보성 질문 답변:** 만약 플레이어의 마지막 행동이 특정 정보(예: '나비는 몇 마리야?', '여기가 어디지?')를 묻는 질문이라면, '마지막 행동 결과'에 담긴 정보에서 해당 질문에 대한 답을 찾아 직접적이고 간결하게 알려주세요. 일반적인 상황 묘사보다는 질문에 대한 답변에 집중해야 합니다. (예: "요원은 그림을 다시 떠올려보았다. 활짝 날개를 펼친 나비는 8마리였다.")
